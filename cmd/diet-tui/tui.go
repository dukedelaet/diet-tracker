...
func (m *model) handleFormKey(k tea.KeyMsg) (tea.Model, tea.Cmd) {
	fmt.Fprintf(os.Stderr, "DEBUG formKey: type=%s str=%q idx=%d form=%d active=%v\n", k.Type, k.String(), m.formIdx, m.form, m.formActive)
	switch {
	case k.Type == tea.KeyCtrlC:
		m.cancelForm()
		return m, m.refreshList()
	...
	case k.Type == tea.KeyEnter, k.Type == tea.KeyCtrlM:
		fmt.Fprintf(os.Stderr, "DEBUG: Enter/CtrlM pressed, calling submitForm\n")
		return m.submitForm()
...
func (m *model) submitForm() (tea.Model, tea.Cmd) {
	...
	case formMeal:
		...
		return m, func() tea.Msg {
			...
			meal, err := m.db.CreateMeal(m.ctx, db.CreateMealParams{Name: name, Protein: int64(p), Carbs: int64(c), Fat: int64(fat), Calories: int64(cals)})
			if err != nil {
				return errMsg{err}
			}
